...
def _construct_criterion(args, nb_classes):
    # DML Losses
    if args.loss == "Proxy_Anchor":
        ...
        criterion = losses.Proxy_NCA(
            nb_classes=nb_classes, sz_embed=args.sz_embedding
        ).cuda()
    elif args.loss == "MS":
...
def main(args):
    ...
    dl_tr, dl_ev, dl_query, dl_gallery, nb_classes = _load_dataset(args)

    model = _construct_model(args)

    criterion = _construct_criterion(args, nb_classes)
    ...
    for epoch in range(0, args.nb_epochs):
        ...
        if bn_freeze:
            modules = (
                model.model.modules()
                if args.gpu_id != -1
                else model.module.model.modules()
            )
            for m in modules:
            ...
            if best_recall[0] < Recalls[0]:
                ...
                with open(
                    "{}/{}_{}_best_results.txt".format(
                        LOG_DIR, args.dataset, args.model
                    ),
                    "w",
                ) as f:
                    f.write("Best Epoch: {}\n".format(best_epoch))
